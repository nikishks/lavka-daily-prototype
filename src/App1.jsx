    ...
    <div id="packs" className="px-4 mb-6 scroll-mt-20">
      ...
      <div className="flex flex-col gap-3 h-[220px] overflow-y-auto snap-y snap-mandatory no-scrollbar rounded-2xl">
        {MOCK_DATA.dietPlans.map((plan) => {
          ...
          // Используем динамические калории для выбранного плана, иначе базовые
          const displayCalories = isSelected ? dynamicCalories : plan.macros.calories;
          return (
            <div 
              key={plan.id}
...
const MealCard = ({ dish }) => (
  <div className="bg-white rounded-[24px] p-3 shadow-sm h-full flex flex-col border border-blue-50 hover:shadow-lg transition-shadow">
    <div className="relative aspect-square mb-3 rounded-2xl overflow-hidden bg-blue-50">
      <img 
        src={dish.image} 
        alt={dish.name} 
        className="w-full h-full object-cover" 
        // Fallback placeholder image for error handling
        onError={(e) => { e.target.onerror = null; e.target.src = `https://placehold.co/400x400/E0F2FE/02ADFF?text=${dish.type.substring(0,2)}`; }}
      />
    </div>
...
export default function App() {
  ...
  // Scroll to ration selector
  const scrollToRation = () => {
